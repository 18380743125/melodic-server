package com.tangl.auth.common.utils;
...
import com.tangl.auth.common.constants.AuthConstants;
...
public class IdUtil {
    ...
    public static Long decrypt(String decryptId) {
        if (StringUtils.isNotBlank(decryptId)) {
            ...
            throw new MelodicBusinessException("解析失败，请检查后重试");
        }
        throw new MelodicBusinessException("the decryptId can not be empty");
    }
    ...
    public static List<Long> decryptIdList(String decryptIdStr) {
        ...
        List<String> decryptIdList = Splitter.on(AuthConstants.COMMON_SEPARATOR).splitToList(decryptIdStr);
        if (CollectionUtil.isEmpty(decryptIdList)) {
            return Lists.newArrayList();
        }
